...
/**
  ...
  *
  * <p> 用于运行任务的调度器
  * <p> 可以重复地运行周期性任务或者在一定延迟时间后执行某个一次性任务
  */
trait Scheduler {
  ...
  /**
    ...
    * @param delay  The amount of time to wait before the first execution 任务首次执行延迟时间
    * @param period The period with which to execute the task. If < 0 the task will execute only once. 执行周期，小于 0 则为一次性任务
    * @param unit   The unit for the preceding times. 前述时间相关的单位
    */
...
/**
  ...
  *
  * <p> 一个基于 java.util.concurrent.ScheduledThreadPoolExecutor 的调度器。
  * <p> 它有一个名称前缀为 kafka-scheduler- 的线程池来执行具体的任务。
  *
  * @param threads          The number of threads in the thread pool 线程池大小
  * @param threadNamePrefix The name to use for scheduler threads. This prefix will have a number appended to it. 线程名前缀
  * @param daemon           If true the scheduler threads will be "daemon" threads and will not block jvm shutdown. 是否将线程池作为后台线程，是的话不会阻止 jvm 关闭。
  */
@threadsafe

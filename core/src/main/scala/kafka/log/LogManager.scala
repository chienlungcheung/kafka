...
/**
  ...
  * <pre>
  * LogManager 是 kafka 日志管理子系统入口，负责日志创建、查询和清理。
  * 所有的读写操作都会被代理给独立的 log 实例。
  *
  * LogManager 维护着一个或多个目录下的日志，新日志在日志最少的目录下创建。
  *
  * 一个后台线程通过周期性地截断超出长度限制的 log segments 来处理 log retention。
  * </pre>
  */
@threadsafe
class LogManager(
      ...
      dir =>
        (
          ...
          new OffsetCheckpointFile(
            new File(dir, RecoveryPointCheckpointFile),
    ...
    .map(
      dir =>
        (
          dir,
          new OffsetCheckpointFile(
            new File(dir, LogStartOffsetCheckpointFile),
            logDirFailureChannel
          )
      )
    )
  ...
  val offlineLogDirectoryCount =
    newGauge("OfflineLogDirectoryCount", new Gauge[Int] {
      def value = offlineLogDirs.size
    })

  for (dir <- logDirs) {
    newGauge("LogDirectoryOffline", new Gauge[Int] {
    ...
    logCreationOrDeletionLock synchronized {
      ...
      info(
        s"Logs for partitions ${offlineCurrentTopicPartitions.mkString(",")} are offline and " +
          s"logs for future partitions ${offlineFutureTopicPartitions
            .mkString(",")} are offline due to failure on log directory $dir"
      )
      dirLocks
  ...
  /**
    ...
    */
  def startup() {
    /* Schedule the cleanup task to delete old logs 安排清理任务来删除老的日志*/
    if (scheduler != null) {
      info(
        "Starting log cleanup with a period of %d ms.".format(retentionCheckMs)
      )
      // 初始延迟为 InitialTaskDelayMs，然后每隔 retentionCheckMs 进行一次日志清理工作
      scheduler.schedule(
        "kafka-log-retention",
        cleanupLogs _,
      ...
      try {
        ...
        case e: Throwable =>
          if (scheduler.isStarted) {
            ...
            error(
              s"Failed to schedule next delete in kafka-delete-logs thread",
              e
            )
          }
      }
  ...
  def replaceCurrentWithFutureLog(topicPartition: TopicPartition): Unit = {
    logCreationOrDeletionLock synchronized {
      ...
      info(
        s"Attempting to replace current log $sourceLog with $destLog for $topicPartition"
      )
      if (sourceLog == null)
        throw new KafkaStorageException(
  ...
  /**
    ...
    * <p>
    * 删除全部可以删除的日志。返回被删除的 segments 个数。
    * 该方法只考虑那些未被压实的日志。
    */
  def cleanupLogs() {
    ...
    val deletableLogs = {
      if (cleaner != null) {
        ...
        // 当针对某个分区正在修改清理策略时，阻止 cleaner 处理该分区，避免 compaction 和 retention 竞争
        cleaner.pauseCleaningForNonCompactedPartitions()
      } else {
        // 过滤出未配置压实的 log，它们才是可删除的
        currentLogs.filter {

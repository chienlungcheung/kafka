...
/**
  ...
  *
  * <p>保存从 timestamp 到某个 segment 中消息的 logical offset 的映射的索引。该索引是稀疏的，意思是，它可能不会为每一个消息保存对应的映射关系。</p>
  *
  ...
  *
  * <p>这个索引会被保存到一个预分配的文件中，每个索引项固定最大 12 个字节。</p>
  *
...
class TimeIndex(_file: File,
  ...
  debug(
    s"Loaded index file ${file.getAbsolutePath} with maxEntries = $maxEntries, maxIndexSize = $maxIndexSize," +
    ...
    maybeLock(lock) {
      if (n >= _entries)
        throw new IllegalArgumentException(
          s"Attempt to fetch the ${n}th entry from  time index ${file.getAbsolutePath} " +
  ...
  /**
    ...
    */
  def maybeAppend(timestamp: Long,
                  offset: Long,
                  skipFullCheck: Boolean = false) {
    inLock(lock) {
      if (!skipFullCheck)
        require(
          !isFull,
          "Attempt to append to a full time index (size = " + _entries + ")."
        )
      ...
      if (_entries != 0 && offset < lastEntry.offset)
        throw new InvalidOffsetException(
          s"Attempt to append an offset ($offset) to slot ${_entries} no larger than" +
            s" the last offset appended (${lastEntry.offset}) to ${file.getAbsolutePath}."
        )
      if (_entries != 0 && timestamp < lastEntry.timestamp)
        throw new IllegalStateException(
          s"Attempt to append a timestamp ($timestamp) to slot ${_entries} no larger" +
      ...
      if (timestamp > lastEntry.timestamp) {
        trace(
          s"Adding index entry $timestamp => $offset to ${file.getAbsolutePath}."
        )
        ...
        require(
          _entries * entrySize == mmap.position(),
          _entries + " entries but file position in index is " + mmap
            .position() + "."
        )
      }
    }
  ...
  def lookup(targetTimestamp: Long): TimestampOffset = {
    maybeLock(lock) {
      ...
      val slot =
        largestLowerBoundSlotFor(idx, targetTimestamp, IndexSearchType.KEY)
      if (slot == -1)
  ...
  private def truncateToEntries(entries: Int) {
    inLock(lock) {
      ...
      _lastEntry = lastEntryFromIndexFile
      debug(
        s"Truncated index ${file.getAbsolutePath} to $entries entries; position is now ${mmap
          .position()} and last entry is now ${_lastEntry}"
      )
    }
  }
...
/**
  ...
  *
  */
class LazyTimeIndex(@volatile private var _file: File,
                    baseOffset: Long,
                    maxIndexSize: Int = -1,
                    writable: Boolean = true) {

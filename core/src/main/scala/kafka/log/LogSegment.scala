...
class LogSegment private[log] (val log: FileRecords,
  ...
  def shouldRoll(rollParams: RollParams): Boolean = {
    val reachedRollMs = timeWaitedForRoll(
      rollParams.now,
      rollParams.maxTimestampInMessages
    ) > rollParams.maxSegmentMs - rollJitterMs
    size > rollParams.maxSegmentBytes - rollParams.messagesSize ||
    (size > 0 && reachedRollMs) ||
    offsetIndex.isFull || timeIndex.isFull || !canConvertToRelativeOffset(
      rollParams.maxOffsetInMessages
    )
  }
  ...
  def sanityCheck(timeIndexFileNewlyCreated: Boolean): Unit = {
    if (lazyOffsetIndex.file.exists) {
      ...
      throw new NoSuchFileException(
        s"Offset index file ${lazyOffsetIndex.file.getAbsolutePath} does not exist"
      )
  }
  ...
  /* The maximum timestamp we see so far 目前我们看到的最大时间戳 */
  @volatile private var _maxTimestampSoFar: Option[Long] = None
  def maxTimestampSoFar_=(timestamp: Long): Unit =
    _maxTimestampSoFar = Some(timestamp)
  def maxTimestampSoFar: Long = {
    if (_maxTimestampSoFar.isEmpty)
      // .timeindex 文件最后一个数据项包含的时间戳
  ...
  def offsetOfMaxTimestampSoFar_=(offset: Long): Unit =
    _offsetOfMaxTimestampSoFar = Some(offset)
  ...
  def append(largestOffset: Long,
    ...
    if (records.sizeInBytes > 0) {
      ...
      trace(
        s"Appended $appendedBytes to ${log.file} at end offset $largestOffset"
      )
  ...
  private def appendChunkFromFile(records: FileRecords,
                                  position: Int,
  ...
  def appendFromFile(records: FileRecords, start: Int): Int = {
    var position = start
    val bufferSupplier: BufferSupplier =
  ...
  private def updateProducerState(producerStateManager: ProducerStateManager,
    ...
    if (batch.hasProducerId) {
      ...
      val appendInfo =
        producerStateManager.prepareUpdate(producerId, isFromClient = false)
  ...
  private[log] def translateOffset(
    offset: Long,
    startingFilePosition: Int = 0
  ): LogOffsetPosition = {
    val mapping = offsetIndex.lookup(offset)
    log.searchForOffsetWithSize(
      offset,
      max(mapping.position, startingFilePosition)
    )
  }
  ...
  def read(startOffset: Long,
    ...
    if (maxSize < 0)
      throw new IllegalArgumentException(
        s"Invalid max size $maxSize for log read from segment $log"
      )
  ...
  def fetchUpperBoundOffset(startOffsetPosition: OffsetPosition,
                            fetchSize: Int): Option[Long] =
    offsetIndex
  ...
  def recover(producerStateManager: ProducerStateManager,
      ...
      for (batch <- log.batches.asScala) {
        ...
        if (batch.magic >= RecordBatch.MAGIC_VALUE_V2) {
          leaderEpochCache.foreach { cache =>
            if (batch.partitionLeaderEpoch > 0 && cache.latestEpoch.forall(
                  batch.partitionLeaderEpoch > _
                ))
    ...
    if (truncated > 0)
      debug(
        s"Truncated $truncated invalid bytes at the end of segment ${log.file.getAbsoluteFile} during recovery"
      )
  ...
  def collectAbortedTxns(fetchOffset: Long,
  ...
  def truncateTo(offset: Long): Int = {
    ...
    val bytesTruncated =
      if (mapping == null) 0 else log.truncateTo(mapping.position)
  ...
  def changeFileSuffixes(oldSuffix: String, newSuffix: String) {
    ...
    offsetIndex.renameTo(
      new File(
        CoreUtils
          .replaceSuffix(lazyOffsetIndex.file.getPath, oldSuffix, newSuffix)
      )
    )
    timeIndex.renameTo(
      new File(
        CoreUtils
          .replaceSuffix(lazyTimeIndex.file.getPath, oldSuffix, newSuffix)
      )
    )
    txnIndex.renameTo(
      new File(
        CoreUtils.replaceSuffix(txnIndex.file.getPath, oldSuffix, newSuffix)
      )
    )
  }
  ...
  /**
    ...
    */
  def findOffsetByTimestamp(
    timestamp: Long,
    startingOffset: Long = baseOffset
  ): Option[TimestampAndOffset] = {
    // Get the index entry with a timestamp less than or equal to the target timestamp
    val timestampOffset = timeIndex.lookup(timestamp)
    val position = offsetIndex
  ...
  /**
    ...
    */
  def deleteIfExists() {
    def delete(delete: () => Boolean,
               fileType: String,
               file: File,
               logIfMissing: Boolean): Unit = {
      try {
        if (delete())
          info(s"Deleted $fileType ${file.getAbsolutePath}.")
        else if (logIfMissing)
          info(
            s"Failed to delete $fileType ${file.getAbsolutePath} because it does not exist."
          )
      } catch {
        case e: IOException =>
          throw new IOException(
            s"Delete of $fileType ${file.getAbsolutePath} failed.",
            e
          )
      }
    }
    ...
    CoreUtils.tryAll(
      Seq(
        ...
        () =>
          delete(
            offsetIndex.deleteIfExists _,
            "offset index",
            lazyOffsetIndex.file,
            logIfMissing = true
        ),
        () =>
          delete(
            timeIndex.deleteIfExists _,
            "time index",
            lazyTimeIndex.file,
            logIfMissing = true
        ),
  ...
  /**
    ...
    * 该 segment 文件对应的最后修改时间对应的 unix 时间戳
    */
  def lastModified = log.file.lastModified

  /**
    * The largest timestamp this segment contains.
    * 该 segment 对应的最大时间戳（.timeindex 文件最后一个数据项包含的时间戳或者 segment 文件最后一次修改时间）
    */
  def largestTimestamp =
    if (maxTimestampSoFar >= 0) maxTimestampSoFar else lastModified
...
object LogSegment {
  ...
  def open(dir: File,
    ...
    new LogSegment(
      FileRecords.open(
        Log.logFile(dir, baseOffset, fileSuffix),
        fileAlreadyExists,
        initFileSize,
        preallocate
      ),
      new LazyOffsetIndex(
        Log.offsetIndexFile(dir, baseOffset, fileSuffix),
        baseOffset = baseOffset,
        maxIndexSize = maxIndexSize
      ),
      new LazyTimeIndex(
        Log.timeIndexFile(dir, baseOffset, fileSuffix),
        baseOffset = baseOffset,
        maxIndexSize = maxIndexSize
      ),
      new TransactionIndex(
        baseOffset,
        Log.transactionIndexFile(dir, baseOffset, fileSuffix)
      ),
      baseOffset,
